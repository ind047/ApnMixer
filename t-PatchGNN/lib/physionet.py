...
class PhysioNet(object):
    ...
    def __init__(
        ...
        if device == torch.device("cpu"):
            data_a = torch.load(
                os.path.join(self.processed_folder, self.set_a), map_location="cpu"
            )
            data_b = torch.load(
                os.path.join(self.processed_folder, self.set_b), map_location="cpu"
            )
            data_c = torch.load(
                os.path.join(self.processed_folder, self.set_c), map_location="cpu"
            )
        else:
                ...
                with open(os.path.join(dirname, txtfile)) as f:
                    ...
                    for l in lines[1:]:
                        ...
                        if param in self.params_dict:
                            ...
                            assert param == "RecordID" or param == "", (
...
def patch_variable_time_collate_fn(
    batch,
    args,
    device=torch.device("cpu"),
    data_type="train",
    ...
    data_dict = utils.split_and_patch_batch(
        data_dict, args, n_observed_tp, patch_indices
    )
...
def variable_time_collate_fn(
    ...
    if args.dataset != "ushcn":
        observed_data = utils.normalize_masked_data(
            observed_data, observed_mask, att_min=data_min, att_max=data_max
        )
        predicted_data = utils.normalize_masked_data(
            predicted_data, predicted_mask, att_min=data_min, att_max=data_max
        )
